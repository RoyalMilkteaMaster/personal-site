...
export async function createPalmProp(gl, mesh, meshSHA256) {
  const hash = async (b) =>
    Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', b)), (x) =>
      x.toString(16).padStart(2, '0'),
    ).join('');
  const manifestBytes = await (
    await fetch('/fantasy/prop-candidate-manifest.json')
  ).arrayBuffer();
  if (
  ...
  const cfgBytes = await (
    await fetch('/fantasy/prop-placement.json')
  ).arrayBuffer();
  ...
  const raw = await load('original-prop.f32'),
    seeds = await load('prop-seeds.f32'),
    order = await load('prop-draw-order.u32');
