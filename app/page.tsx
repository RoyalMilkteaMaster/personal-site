...
export default function Home(){
 ...
 const [introStep,setIntroStep]=useState('loading');
 ...
 <aside className="portrait-stage"><div className="stage-label">ROYAL MILKTEA / PERSONAL UNIVERSE</div><AstralScene pose={Number(active)} replay={replay} skip={skip} onIntroStep={setIntroStep}/><div className="stage-controls"><button onClick={()=>{setActive('0');setSkip(false);setReplay(replay+1);}}><RotateCcw size={14}/>重播開場</button><button onClick={()=>setSkip(true)}>略過開場 →</button></div><div className="stage-caption"><span>0{Number(active)+1}</span><span>{sections[Number(active)]}</span><i>✧</i></div></aside>
 <div ref={contentRef} className="atlas-content" id="content" data-intro={active==='0'?introStep:'done'}><p className="eyebrow">林品宏 ／ 皇家奶茶大師</p><TabsList className="chapter-tabs" aria-label="個人網站主題">{sections.map((s,i)=><TabsTrigger key={s} value={String(i)}><span>0{i+1}</span>{s}</TabsTrigger>)}</TabsList>
 {active==='0'&&['loading','hair','face'].includes(introStep)&&<div className="intro-copy" key={introStep==='face'?'face':'hair'}><p className="chapter-kicker">ROYAL MILKTEA MASTER</p><h1>{introStep==='face'?<>嗨，我是<br/><em>林品宏。</em></>:<>好奇，<br/><em>是我的起點。</em></>}</h1><p className="lead">{introStep==='face'?'探索 AI、自動化與系統開發，讓想法成為能被使用的作品。':'從一個想法，走向下一種可能。'}</p></div>}
